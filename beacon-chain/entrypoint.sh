...
echo "Starting beacon node with chain ${CHAIN} and node type ${NODE_TYPE}"
if [ "$NODE_TYPE" == "Full" ]; then
    EXTRA_OPTS="${EXTRA_OPTS} --prune.mode=full"
elif [ "$NODE_TYPE" == "Archive" ]; then
    EXTRA_OPTS="${EXTRA_OPTS} --prune.mode=archive"
elif [ "$NODE_TYPE" == "Minimal" ]; then
    EXTRA_OPTS="${EXTRA_OPTS} --prune.mode=minimal"
fi
